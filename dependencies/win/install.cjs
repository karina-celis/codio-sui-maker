...
const install = async () => {
  ...
  const download = `${GITHUB_REPO}/releases/download/v1.0.2/${filename}`;
...
const {
  platform,
  arch,
  env: { SKIP_NTSUSPEND_BINARY },
} = process;
if (!SKIP_NTSUSPEND_BINARY && platform === 'win32' && ['x64', 'ia32'].indexOf(arch) >= 0) install();
