...
import { isDocumentChangeEvent, isEditorEvent } from '../event_creator';
...
export function getInitialFilePathAndContentFromFrame(initialFrame: CodioFrame): IntitialFileContent {
...
export function createFrame(initialFrame: Array<CodioFile>, timeline: Array<CodioEvent>): CodioFrame {
  ...
  timeline.forEach((event) => {
    if (isDocumentChangeEvent(event) && event.data.uri) {
